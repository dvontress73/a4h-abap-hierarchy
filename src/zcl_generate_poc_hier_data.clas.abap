CLASS zcl_generate_poc_hier_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .

  PUBLIC SECTION.
    TYPES: ty_replytable TYPE STANDARD TABLE OF zreplytable WITH KEY replytable_uuid,
           ty_replycode  TYPE STANDARD TABLE OF zreplycode WITH KEY reply_code_uuid,
           ty_mrc_header TYPE STANDARD TABLE OF zmrc WITH KEY mrc_uuid.
...
METHOD if_oo_adt_classrun~main.
    ...
    DELETE FROM zmrc.
    DELETE FROM zreplytable.
    DELETE FROM zreplycode.
    "DELETE FROM zmrc_build.
    "DELETE FROM zmrcbuildd1.
    "DELETE FROM zmrc_buildd.
    "DELETE FROM zreplytable_d.
    "DELETE FROM zzreplycode_00d.
    "DELETE FROM zreplytable00d.
    "DELETE FROM zreplycoded.
    "DELETE FROM zreplytabled.
    "DELETE FROM zreplytable_d.
    "DELETE FROM zmrc_d.

    fill_replytable(  ).
    INSERT zreplytable FROM TABLE @t_replytable.
    COMMIT WORK.
    out->write( |Replytable data created| ).

    fill_replycode( t_replytable ).
    INSERT zreplycode FROM TABLE @t_replycode.
    COMMIT WORK.
    out->write( |Replycode data created| ).

    fill_mrc_table(  ).
    INSERT zmrc FROM TABLE @t_mrc_header.
    COMMIT WORK.
  ...
  METHOD fill_replycode.
    ...
    IF sy-subrc = 0.
      ...
      "ID = xxx1
      wa_code-reply_code_uuid = zcl_get_uuid_x16=>generate_uuid_x16(  ).
      wa_code-code = to_upper( 'xxx1' ).
      wa_code-description = to_upper( 'core conductor first conductor' ).
      APPEND wa_code TO t_replycode.

      "ID = xxx2
      wa_code-reply_code_uuid = zcl_get_uuid_x16=>generate_uuid_x16(  ).
      wa_code-code = to_upper( 'xxx2' ).
      wa_code-description = to_upper( 'core conductor second conductor' ).
      APPEND wa_code TO t_replycode.
